...
public class WebSocketTest extends VertxTestBase {
  ...
  public void testTLSClientTrustServerCertWithSNI() throws Exception {
    testTLS(Cert.NONE, Trust.SNI_JKS_HOST2, Cert.SNI_JKS, Trust.NONE, false, false, false, false, true, true, true, true, new String[0],
        client -> Future.future(h -> client.websocket(4043, "host2.com", "/", h)));
  }
  ...
  public void testClearClientRequestOptionsSetSSL() throws Exception {
    ...
    testTLS(Cert.NONE, Trust.NONE, Cert.SERVER_JKS, Trust.NONE, false, false, true, false, true, false, true, false, new String[0], client -> Future.future(h -> client.websocket(options, h)));
  }
  ...
  public void testSSLClientRequestOptionsSetSSL() throws Exception {
    RequestOptions options = new RequestOptions().setHost(HttpTestBase.DEFAULT_HTTP_HOST).setURI("/").setPort(4043).setSsl(true);
    testTLS(Cert.NONE, Trust.NONE, Cert.SERVER_JKS, Trust.NONE, false, false, true, false, true, true, true, false, new String[0], client -> Future.future(h -> client.websocket(options, h)));
  }
  ...
  public void testClearClientRequestOptionsSetClear() throws Exception {
    ...
    testTLS(Cert.NONE, Trust.NONE, Cert.SERVER_JKS, Trust.NONE, false, false, true, false, true, false, false, false, new String[0], client -> Future.future(h -> client.websocket(options, h)));
  }
  ...
  public void testSSLClientRequestOptionsSetClear() throws Exception {
    RequestOptions options = new RequestOptions().setHost(HttpTestBase.DEFAULT_HTTP_HOST).setURI("/").setPort(4043).setSsl(false);
    testTLS(Cert.NONE, Trust.NONE, Cert.SERVER_JKS, Trust.NONE, false, false, true, false, true, true, false, false, new String[0], client -> Future.future(h -> client.websocket(options, h)));
  }
  ...
  private void testTLS(Cert<?> clientCert, Trust<?> clientTrust,
    ...
    testTLS(clientCert, clientTrust,
        ...
        enabledCipherSuites, client -> Future.future(h -> client.websocket(4043, HttpTestBase.DEFAULT_HTTP_HOST, "/", h)));
  }
  ...
  private void testTLS(Cert<?> clientCert, Trust<?> clientTrust,
    ...
    try {
      server.listen(ar -> {
        ...
        wsProvider.apply(client).setHandler(res -> {
          if (res.succeeded()) {
            wsHandler.handle(res.result());
          } else {
            errHandler.handle(res.cause());
          }
        });
      });
    } catch (Exception e) {
  ...
  private void testWSWriteStream(WebsocketVersion version) throws Exception {
    ...
    server.listen(ar -> {
      ...
      client.websocket(DEFAULT_HTTP_PORT, HttpTestBase.DEFAULT_HTTP_HOST, path + "?" + query, null, version, onSuccess(ws -> {
  ...
  private void testContinuationWriteFromConnectHandler(WebsocketVersion version) throws Exception {
    ...
    server.listen(ar -> {
      assertTrue(ar.succeeded());
      client.websocket(DEFAULT_HTTP_PORT, HttpTestBase.DEFAULT_HTTP_HOST, path, null, version, onSuccess(ws -> {
  ...
  private void testValidSubProtocol(WebsocketVersion version) throws Exception {
    ...
    server.listen(ar -> {
      assertTrue(ar.succeeded());
      client.websocket(DEFAULT_HTTP_PORT, HttpTestBase.DEFAULT_HTTP_HOST, path, null, version, clientSubProtocols, onSuccess(ws -> {
        assertEquals("commonproto", ws.subProtocol());
        final Buffer received = Buffer.buffer();
        ws.handler(data -> {
          received.appendBuffer(data);
  ...
  private void testInvalidSubProtocol(WebsocketVersion version) throws Exception {
    ...
    server.listen(onSuccess(ar -> {
      client.websocket(DEFAULT_HTTP_PORT, HttpTestBase.DEFAULT_HTTP_HOST, path, null, version, subProtocol, onFailure(err -> {
        // Should fail
        testComplete();
      }));
    }));
  ...
  private void testReject(WebsocketVersion version, Integer rejectionStatus, int expectedRejectionStatus) throws Exception {
    ...
    server.listen(ar -> {
      assertTrue(ar.succeeded());
      client.websocket(DEFAULT_HTTP_PORT, HttpTestBase.DEFAULT_HTTP_HOST, path, null, version, onFailure(t -> {
  ...
  private void testCloseCallHandlers(boolean local) {
    ...
    server.listen(ar -> {
      assertTrue(ar.succeeded());
      client.websocket(DEFAULT_HTTP_PORT, HttpTestBase.DEFAULT_HTTP_HOST, path, null, onSuccess(ws -> {
        ws.closeHandler(v -> {
          testComplete();
        });
        if (local) {
          vertx.runOnContext(v -> ws.close());
        }
      }));
    });
    await();
  ...
  public void httpClientWebsocketConnectionFailureHandlerShouldBeCalled() throws Exception {
    ...
    client.websocket(port, "localhost", "", onFailure(throwable -> testComplete()));
    await();
  }
  ...
  private void doTestClientWebsocketConnectionCloseOnBadResponse(boolean keepAliveInOptions) throws Throwable {
    ...
    netServer = vertx.createNetServer().connectHandler(sock -> {
      ...
      client.close();
      client = vertx.createHttpClient(opts).websocket(port, "localhost", "/", ar2 -> {
        if (ar2.succeeded()) {
  ...
  public void testClearClientSslOptions() {
    ...
    server = vertx.createHttpServer(serverOptions).websocketHandler(WebSocketBase::close).listen(onSuccess(server -> {
      ...
      client.websocket(requestOptions, onSuccess(ws -> {
        ws.closeHandler(v -> {
          testComplete();
        });
      }));
  ...
  public void testReportProtocolViolationOnClient() {
    ...
    server.listen(ar -> {
      assertTrue(ar.succeeded());
      client.websocket(DEFAULT_HTTP_PORT, HttpTestBase.DEFAULT_HTTP_HOST, "/some/path", null, WebsocketVersion.V13, onSuccess(ws -> {
